...
def enviar_temporal(chat_id, texto, markup=None, parse_mode="Markdown"):
    try:
        ...
        auto_destruir_mensaje(chat_id, [msg.message_id], TIEMPO_AUTO_ELIMINAR)
        return msg
    except Exception as e:
        return None
...
@bot.message_handler(commands=['start', 'hvn94'])
def cmd_start_hvn94(message):
    ...
    if not es_miembro_autorizado(message.from_user.id):
        enviar_temporal(message.chat.id, "⛔ **Acceso Restringido**.\n\nNo estás autorizado para usar este bot.")
        return
    enviar_temporal(message.chat.id, "👋 **¡Panel HVN94 Convocado!**\n\nUsa los botones de abajo para navegar por el catálogo.", markup=teclado_principal())
...
def ayuda(message):
    if not es_miembro_autorizado(message.from_user.id):
        return
    enviar_temporal(message.chat.id, f"ℹ️ **Ayuda HVN94**\n\n📁 Catálogo Completo: ver todos los packs.\n🕒 Últimas Subidas: próximamente.\n\n👤 Contacto: {CONTACTO_ADMIN}")

@bot.message_handler(content_types=['document', 'video', 'audio', 'photo'], func=lambda msg: msg.chat.type in ['group', 'supergroup'])
...
def callbacks(call):
    try:
        ...
        if data.startswith("descargar_"):
            ...
            detalles = obtener_detalles_pack(pack_id)
            if not detalles:
                bot.answer_callback_query(call.id, "Pack no encontrado.")
                return
            _, archivos = detalles
            for file_id, nombre, tipo in archivos:
                try:
                    if tipo == "document": bot.send_document(call.message.chat.id, file_id)
                    elif tipo == "video": bot.send_video(call.message.chat.id, file_id)
                    elif tipo == "audio": bot.send_audio(call.message.chat.id, file_id)
                    elif tipo == "photo": bot.send_photo(call.message.chat.id, file_id)
                except Exception:
                    pass
            bot.answer_callback_query(call.id, "Pack enviado.")
            return
